...
export function PostHero({ post }: PostHeroProps) {
  ...
  const coverImgUrl = attributes.coverImg?.data?.attributes?.url
    ? `${process.env.NEXT_PUBLIC_API_BASE_URL || "https://lite-tech-api.litebox.ai"}${attributes.coverImg.data.attributes.url}`
  ...
  return (
    <div className="relative flex items-end h-125 md:h-145 lg:h-160 p-6 md:p-14 lg:p-30">
      <Image
        src={coverImgUrl}
        alt={
          attributes.coverImg?.data?.attributes?.alternativeText ||
          "Article hero"
        }
        fill
          ...
          <div className="bg-white pb-0 p-5">
            <div className="flex items-center gap-2">
              <Avatar className="size-10">
                ...
                <AvatarFallback>
                  {attributes.author.slice(0, 2).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <span className="text-border">By {attributes.author}</span>
            </div>
          </div>
